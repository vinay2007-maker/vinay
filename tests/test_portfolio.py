...
def test_long_and_short_pnl():
    ...
    assert p.close_position(0, 110) == pytest.approx(20)
    p.open_position('SHORT', 100, 2, 110, 80)
    assert p.close_position(0, 90) == pytest.approx(20)
...
def test_fees():
    ...
    assert p.entry_fees == pytest.approx(.20)
    assert p.exit_fees == pytest.approx(.22)
    assert p.fees == pytest.approx(.42)
    assert p.gross_realized_pnl == pytest.approx(20)
    assert p.realized_pnl == pytest.approx(19.58)
