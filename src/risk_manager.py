...
class TradePlan:
    signal: Signal; entry: float; quantity: float; stop_loss: float; take_profit: float; risk_amount: float

class RiskManager:
    def __init__(self, risk_per_trade=.01, max_daily_loss=.03, max_position_size=1.0, max_open_positions=2, risk_reward_ratio=2.0):
        if not 0 < risk_per_trade <= .01: raise ValueError("risk_per_trade must be in (0, 0.01]")
        if not 0 < max_daily_loss <= 1 or max_position_size <= 0 or max_open_positions < 0 or risk_reward_ratio <= 0: raise ValueError("invalid risk settings")
        ...
        self.risk_reward_ratio, self.kill_switch = risk_reward_ratio, False
    ...
    def create_plan(self, signal, equity, entry, stop_loss, take_profit=None, open_positions=0, daily_loss=0.0):
        ...
        quantity = min(risk_amount / distance, self.max_position_size)
        if quantity <= 0: raise ValueError("quantity is not positive")
        return TradePlan(signal, entry, quantity, stop_loss, take_profit, quantity * distance)
